...
- Treat `.env.schema` as the environment-variable source of truth and use `ENV` from `varlock/env` in application code. Never read local env files; validate safely with `vp exec varlock load --agent`.
